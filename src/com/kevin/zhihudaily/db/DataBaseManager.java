...
public class DataBaseManager {
	...
	public int writeNewsToDB(NewsModel model) {
		...
		try {
			...
			db.updateWithOnConflict(DataBaseConstants.NEWS_TABLE_NAME, values,
					DataBaseConstants.ID + "=?", whereArgs,
	...
	public int updateNewsBodyToDB(int id, String body) {
		...
		try {
			...
			count = db.updateWithOnConflict(DataBaseConstants.NEWS_TABLE_NAME,
					values, DataBaseConstants.ID + "=?", whereArgs,
					SQLiteDatabase.CONFLICT_REPLACE);
			db.setTransactionSuccessful();
		} catch (Exception e) {
			// TODO: handle exception
	...
	public DailyNewsModel readDaliyNewsList(String date) {
		...
		String[] columns = { DataBaseConstants.ID, DataBaseConstants.DATE,
				DataBaseConstants.GA_PREFIX, DataBaseConstants.IS_TOP_STORY,
				DataBaseConstants.TITLE, DataBaseConstants.URL,
				DataBaseConstants.IMAGE_SOURCE, DataBaseConstants.IMAGE_URL,
				DataBaseConstants.IMAGE_THUMBNAIL, DataBaseConstants.SHARE_URL,
				DataBaseConstants.BODY };
		String selection = "date=?";
		String[] selectionArgs = { date };
		String orderBy = DataBaseConstants.GA_PREFIX + " DESC" + ", "
		...
		if (cursor != null && cursor.getCount() > 0) {
			...
			while (cursor.moveToNext()) {
				...
				model.setGa_prefix(cursor.getString(cursor
						.getColumnIndexOrThrow(DataBaseConstants.GA_PREFIX)));
				...
				// read body
				model.setBody(cursor.getString(cursor
						.getColumnIndexOrThrow(DataBaseConstants.BODY)));
			...
			try {
				...
				SimpleDateFormat diaplayFormat = new SimpleDateFormat(
						"yyyy.M.d cccc");
