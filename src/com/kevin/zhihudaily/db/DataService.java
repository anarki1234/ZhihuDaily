...
public class DataService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		case Constants.ACTION_WRITE_DAILY_NEWS:
			...
			DailyNewsModel model = DataCache.getInstance().getDailyNewsModel(
					key);
		...
		case Constants.ACTION_READ_DAILY_NEWS:
			...
			DailyNewsModel dailyNewsModel = DataBaseManager.getInstance()
					.readDaliyNewsList(date);
			if (dailyNewsModel != null) {
				DataCache.getInstance().addDailyCache(dailyNewsModel.getDate(),
						dailyNewsModel);
		...
		case Constants.ACTION_READ_NEWS_DEATIL:
			...
			String news_date = intent
					.getStringExtra(Constants.INTENT_NEWS_DATE);
			if (news_date == null || news_id == -1) {
				break;
			}
			String news_body = DataBaseManager.getInstance().readNewsBody(
					news_id);
			if (news_body != null) {
				// Update to cache
				DataCache.getInstance().updateNewsBodyByID(news_date, news_id,
						news_body);
